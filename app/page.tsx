...
"use client";
import HeroCarousel from '../components/herocarousel';
import { ArrowRight, Heart, Stethoscope, Users, Clock, Pill, Activity, Zap, Award, Calendar, FileText } from 'lucide-react';
import Link from 'next/link';
import { useEffect, useState } from 'react';
import Layout from '../components/layout'
      ...
      <section className={`relative py-24 transition-colors duration-300 ${isDark ? 'bg-slate-950/50' : 'bg-gradient-to-b from-gray-50 to-white'}`}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: Award, title: 'Certified Specialists', description: 'All doctors are board-certified with international qualifications' },
              { icon: FileText, title: 'Online Report Access', description: 'Access your medical reports anytime, anywhere through our secure online portal' },
              { icon: Clock, title: '24/7 365 Days Always Open', description: 'Our clinic is open round-the-clock every single day of the year for all medical services' },
              { icon: Zap, title: 'Quick Diagnosis', description: 'Advanced diagnostic tools for fast and accurate results' },
              { icon: Heart, title: 'Patient Care', description: 'Personalized treatment plans tailored to your needs' },
              { icon: Activity, title: 'Modern Equipment', description: 'State-of-the-art medical technology for better treatment outcomes' },
            ].map((feature, index) => {
              const Icon = feature.icon;
              return (
                <div
