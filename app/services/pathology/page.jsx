"use client";
import { Beaker, Award, Clock, Users, Phone, Mail, MapPin, ArrowRight, Check, Zap, Activity, Calendar, FileText, TrendingUp, TestTube } from 'lucide-react';
import Link from 'next/link';
import { useState, useEffect } from 'react';
import Layout from '../../../components/layout'
export default function Pathology() {
  const [isDark, setIsDark] = useState(true);
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    setMounted(true);
    const updateTheme = () => {
      const isDarkMode = document.documentElement.classList.contains('dark');
      setIsDark(isDarkMode);
    };

    updateTheme();
    window.addEventListener('storage', updateTheme);
    
    const observer = new MutationObserver(updateTheme);
    observer.observe(document.documentElement, { attributes: true, attributeFilter: ['class'] });

    return () => {
      window.removeEventListener('storage', updateTheme);
      observer.disconnect();
    };
  }, []);

  if (!mounted) return null;

  const clinicalPathologyServices = [
    {
      name: 'Complete Blood Count (CBC)',
      description: 'Analysis of red blood cells, white blood cells, and platelets with differential count for infection and hematologic disorders assessment'
    },
    {
      name: 'Blood Chemistry Panel',
      description: 'Comprehensive metabolic assessment including electrolytes, renal function, liver function, and glucose levels'
    },
    {
      name: 'Coagulation Studies',
      description: 'PT, aPTT, INR measurement for bleeding disorders and anticoagulation monitoring'
    },
    {
      name: 'Lipid Profile',
      description: 'Total cholesterol, LDL, HDL, and triglyceride measurement for cardiovascular risk assessment'
    },
    {
      name: 'Thyroid Function Tests',
      description: 'TSH, Free T3, Free T4 measurement for thyroid disorder diagnosis and management'
    },
    {
      name: 'Blood Gas Analysis',
      description: 'pH, pO2, pCO2, HCO3 measurement for respiratory and metabolic function assessment'
    },
    {
      name: 'Cardiac Biomarkers',
      description: 'Troponin, CK-MB, BNP measurement for myocardial infarction and heart failure detection'
    },
    {
      name: 'Tumor Markers',
      description: 'PSA, CEA, CA-125 measurement for cancer screening and monitoring'
    }
  ];

  const microbiologyServices = [
    {
      name: 'Bacterial Culture & Sensitivity',
      description: 'Isolation and identification of pathogenic bacteria with antimicrobial susceptibility testing'
    },
    {
      name: 'Fungal Culture',
      description: 'Identification of fungal pathogens with species-specific antifungal susceptibility'
    },
    {
      name: 'Viral Serology',
      description: 'HIV, Hepatitis B/C, Dengue, and other viral antibody/antigen detection'
    },
    {
      name: 'Urinalysis',
      description: 'Physical, chemical, and microscopic analysis of urine for urinary tract disorders'
    },
    {
      name: 'Stool Examination',
      description: 'Parasitic and bacterial examination for gastrointestinal infections'
    },
    {
      name: 'Sexually Transmitted Infections (STI)',
      description: 'Testing for gonorrhea, chlamydia, syphilis, and other sexually transmitted pathogens'
    }
  ];

  const serologicalTests = [
    {
      test: 'ABO & Rh Typing',
      description: 'Blood group determination for transfusion compatibility and prenatal assessment'
    },
    {
      test: 'Rheumatoid Factor & ANA',
      description: 'Autoimmune disease markers for rheumatoid arthritis and lupus screening'
    },
    {
      test: 'Hepatitis Panel',
      description: 'HBsAg, HBeAg, Anti-HCV for hepatitis B and C screening'
    },
    {
      test: 'HIV Antibody Test',
      description: 'Enzyme Immunoassay (EIA) with confirmatory testing algorithm'
    },
    {
      test: 'Widal Test',
      description: 'Typhoid agglutinins detection for enteric fever diagnosis'
    },
    {
      test: 'RPR/Syphilis Screening',
      description: 'Rapid Plasma Reagin testing for syphilis with VDRL confirmation'
    }
  ];

  const diagnosticApplications = [
    {
      category: 'Infectious Diseases',
      examples: 'Tuberculosis, malaria, dengue, typhoid, UTI, respiratory infections'
    },
    {
      category: 'Hematologic Disorders',
      examples: 'Anemia, leukemia, lymphoma, bleeding disorders, thrombocytopenia'
    },
    {
      category: 'Metabolic Disorders',
      examples: 'Diabetes, thyroid disorders, renal disease, liver disease'
    },
    {
      category: 'Cardiovascular Assessment',
      examples: 'Hypertension, dyslipidemia, acute coronary syndrome, heart failure'
    },
    {
      category: 'Cancer Screening',
      examples: 'Prostate cancer (PSA), colorectal cancer (CEA), ovarian cancer (CA-125)'
    },
    {
      category: 'Pregnancy Monitoring',
      examples: 'Blood type, infectious disease screening, glucose tolerance, anemia assessment'
    }
  ];

  const qualityAssurance = [
    {
      measure: 'Pre-Analytical Quality',
      items: [
        'Proper specimen collection and labeling protocols',
        'Chain of custody documentation',
        'Temperature-controlled specimen transport',
        'Quality control of collection tubes'
      ]
    },
    {
      measure: 'Analytical Quality',
      items: [
        'ISO 15189 accreditation standards compliance',
        'Daily calibration and quality control runs',
        'Internal and external quality assurance programs',
        'Regular equipment maintenance and verification'
      ]
    },
    {
      measure: 'Post-Analytical Quality',
      items: [
        'Result verification by qualified pathologists',
        'Reflex testing protocols for abnormal results',
        'Comprehensive report generation with reference ranges',
        'Critical value notification procedures'
      ]
    }
  ];

  const turnaroundTimes = [
    {
      testCategory: 'Routine Tests',
      time: '24-48 hours',
      examples: 'CBC, chemistry panel, urinalysis'
    },
    {
      testCategory: 'Urgent Tests',
      time: '4-6 hours',
      examples: 'Cardiac biomarkers, blood glucose, electrolytes'
    },
    {
      testCategory: 'STAT Tests',
      time: '1-2 hours',
      examples: 'Blood gas, troponin, critical coagulation studies'
    },
    {
      testCategory: 'Culture Tests',
      time: '3-7 days',
      examples: 'Bacterial culture, fungal culture'
    }
  ];

  const teamMembers = [
    {
      name: 'Dr. Sukhransh Mazumdar',
      qualification: 'MD (Pathology), DNB',
      specialization: 'Clinical & Laboratory Pathology',
      experience: '12+ Years',
      expertise: [
        'Clinical Chemistry & Hematology',
        'Immunoassays & Serology',
        'Microbiology & Culture Techniques',
        'Quality Assurance & Accreditation',
        'Laboratory Management & Optimization'
      ],
      availability: 'Mon & Sat 5:00 PM'
    }
  ];

  return (
    <Layout>
    <div className={`overflow-hidden transition-colors duration-300 ${isDark ? 'bg-slate-950' : 'bg-white'}`}>
      {/* ============ HERO SECTION ============ */}
      <section className={`relative min-h-[70vh] flex items-center px-4 sm:px-6 lg:px-8 pt-32 overflow-hidden transition-colors duration-300 ${isDark ? 'bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950' : 'bg-gradient-to-br from-white via-yellow-50/40 to-amber-50/30'}`}>
        <div className="absolute inset-0 overflow-hidden">
          <div className={`absolute -top-40 right-0 w-96 h-96 rounded-full blur-3xl animate-pulse ${isDark ? 'bg-yellow-500/20' : 'bg-yellow-300/20'}`}></div>
          <div className={`absolute -bottom-40 left-1/2 w-96 h-96 rounded-full blur-3xl animate-pulse ${isDark ? 'bg-amber-500/20' : 'bg-amber-300/20'}`}></div>
        </div>

        <div className="relative max-w-7xl mx-auto w-full">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border backdrop-blur-sm mb-6 transition-all duration-300 ${isDark ? 'bg-emerald-500/15 border-emerald-500/30' : 'bg-emerald-100/70 border-emerald-300/60'}`}>
                <TestTube className={`w-4 h-4 ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`} />
                <span className={`text-sm font-semibold ${isDark ? 'text-emerald-300' : 'text-emerald-700'}`}>PATHOLOGY DEPARTMENT</span>
              </div>

              <h1 className={`text-6xl lg:text-7xl font-black mb-6 leading-tight transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                Advanced
                <span className="block bg-gradient-to-r from-emerald-600 via-cyan-600 to-emerald-600 bg-clip-text text-transparent">
                  Diagnostic Labs
                </span>
              </h1>

              <p className={`text-xl transition-colors duration-300 mb-8 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                Comprehensive laboratory diagnostic services utilizing advanced analytical techniques with emphasis on accuracy, precision, and rapid turnaround times.
              </p>

              <div className="flex flex-wrap gap-6 mb-8">
                <div>
                  <p className="text-4xl font-black bg-gradient-to-r from-emerald-600 to-cyan-600 bg-clip-text text-transparent">12+</p>
                  <p className={`text-sm font-medium transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Years Experience</p>
                </div>
                <div>
                  <p className="text-4xl font-black bg-gradient-to-r from-emerald-600 to-cyan-600 bg-clip-text text-transparent">100K+</p>
                  <p className={`text-sm font-medium transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Tests Performed</p>
                </div>
                <div>
                  <p className="text-4xl font-black bg-gradient-to-r from-emerald-600 to-cyan-600 bg-clip-text text-transparent">ISO</p>
                  <p className={`text-sm font-medium transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Accredited</p>
                </div>
              </div>

              <Link
                href="/"
                className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-emerald-600 to-cyan-600 text-white font-bold rounded-full hover:shadow-2xl transition-all duration-300 hover:scale-105 group"
              >
                <Calendar className="w-5 h-5" />
                Book Test
                <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>

            <div className="hidden lg:flex items-center justify-center">
              <div className="relative w-80 h-80 rounded-3xl flex items-center justify-center text-9xl bg-gradient-to-br from-emerald-600 to-cyan-600 shadow-2xl shadow-emerald-600/30">
                <Beaker className="w-48 h-48 text-white" />
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* ============ OVERVIEW SECTION ============ */}
      <section className={`relative py-24 px-4 sm:px-6 lg:px-8 transition-colors duration-300 ${isDark ? 'bg-slate-900/50' : 'bg-gradient-to-b from-white via-emerald-50/30 to-white'}`}>
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-3 gap-8">
            {[
              {
                icon: Zap,
                title: 'Accuracy & Precision',
                desc: 'ISO 15189 accredited laboratory with rigorous quality control and validated methodologies'
              },
              {
                icon: TrendingUp,
                title: 'Advanced Technology',
                desc: 'State-of-the-art automated analyzers and modern diagnostic equipment'
              },
              {
                icon: Users,
                title: 'Expert Interpretation',
                desc: 'Comprehensive analysis and clinical correlation by experienced pathologists'
              }
            ].map((item, idx) => {
              const Icon = item.icon;
              return (
                <div
                  key={idx}
                  className={`rounded-2xl border p-8 text-center transition-all duration-300 hover:-translate-y-2 ${
                    isDark
                      ? 'bg-white/5 border-white/10 hover:border-emerald-500/50'
                      : 'bg-white border-gray-200 shadow-sm hover:shadow-md'
                  }`}
                >
                  <Icon className="w-12 h-12 text-emerald-600 mx-auto mb-4" />
                  <h3 className={`text-xl font-bold mb-3 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    {item.title}
                  </h3>
                  <p className={`transition-colors duration-300 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                    {item.desc}
                  </p>
                </div>
              );
            })}
          </div>
        </div>
      </section>

      {/* ============ CLINICAL PATHOLOGY SERVICES ============ */}
      <section className={`relative py-24 px-4 sm:px-6 lg:px-8 transition-colors duration-300 ${isDark ? 'bg-slate-950' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto">
          <div className="mb-16">
            <h2 className={`text-5xl font-black mb-6 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
              Clinical Pathology Services
            </h2>
            <p className={`text-lg transition-colors duration-300 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
              Comprehensive blood and body fluid analysis
            </p>
          </div>

          <div className="grid md:grid-cols-2 gap-6">
            {clinicalPathologyServices.map((service, idx) => (
              <div
                key={idx}
                className={`rounded-2xl border p-6 transition-all duration-300 hover:-translate-y-1 ${
                  isDark
                    ? 'bg-white/5 border-white/10 hover:border-emerald-500/50'
                    : 'bg-gradient-to-br from-emerald-50 to-cyan-50 border-emerald-200/60 shadow-sm hover:shadow-md'
                }`}
              >
                <div className="flex gap-4 items-start">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-gradient-to-br from-emerald-600 to-cyan-600 flex-shrink-0 mt-1">
                    <Check className="w-6 h-6 text-white" />
                  </div>
                  <div>
                    <h3 className={`text-lg font-bold mb-2 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {service.name}
                    </h3>
                    <p className={`text-sm transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-700'}`}>
                      {service.description}
                    </p>
                  </div>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ============ MICROBIOLOGY SERVICES ============ */}
      <section className={`relative py-24 px-4 sm:px-6 lg:px-8 transition-colors duration-300 ${isDark ? 'bg-slate-900/50' : 'bg-gray-50'}`}>
        <div className="max-w-7xl mx-auto">
          <div className="mb-16">
            <h2 className={`text-5xl font-black mb-6 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
              Microbiology Services
            </h2>
            <p className={`text-lg transition-colors duration-300 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
              Culture, isolation, and identification of infectious agents
            </p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {microbiologyServices.map((service, idx) => (
              <div
                key={idx}
                className={`rounded-2xl border p-6 transition-all duration-300 ${
                  isDark
                    ? 'bg-white/5 border-white/10 hover:border-emerald-500/50'
                    : 'bg-white border-gray-200 hover:border-emerald-400 shadow-sm hover:shadow-md'
                }`}
              >
                <div className="flex gap-4 items-start">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-gradient-to-br from-emerald-600 to-cyan-600 flex-shrink-0 mt-1">
                    <Activity className="w-6 h-6 text-white" />
                  </div>
                  <div>
                    <h3 className={`text-lg font-bold mb-2 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {service.name}
                    </h3>
                    <p className={`text-sm transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-700'}`}>
                      {service.description}
                    </p>
                  </div>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ============ SEROLOGICAL TESTS ============ */}
      <section className={`relative py-24 px-4 sm:px-6 lg:px-8 transition-colors duration-300 ${isDark ? 'bg-slate-950' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto">
          <div className="mb-16">
            <h2 className={`text-5xl font-black mb-6 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
              Serological & Immunological Tests
            </h2>
            <p className={`text-lg transition-colors duration-300 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
              Antibody and antigen detection for infectious disease screening
            </p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {serologicalTests.map((test, idx) => (
              <div
                key={idx}
                className={`rounded-2xl border p-6 transition-all duration-300 ${
                  isDark
                    ? 'bg-white/5 border-white/10 hover:border-emerald-500/50'
                    : 'bg-gradient-to-br from-emerald-50 to-cyan-50 border-emerald-200/60 shadow-sm'
                }`}
              >
                <h3 className={`text-lg font-bold mb-3 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {test.test}
                </h3>
                <p className={`text-sm transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-700'}`}>
                  {test.description}
                </p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ============ DIAGNOSTIC APPLICATIONS ============ */}
      <section className={`relative py-24 px-4 sm:px-6 lg:px-8 transition-colors duration-300 ${isDark ? 'bg-slate-900/50' : 'bg-gray-50'}`}>
        <div className="max-w-7xl mx-auto">
          <h2 className={`text-5xl font-black mb-16 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
            Clinical Applications
          </h2>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {diagnosticApplications.map((app, idx) => (
              <div
                key={idx}
                className={`rounded-2xl border p-8 transition-all duration-300 ${
                  isDark
                    ? 'bg-gradient-to-br from-white/10 to-white/5 border-white/15'
                    : 'bg-white border-gray-200 shadow-sm'
                }`}
              >
                <h3 className={`text-xl font-bold mb-4 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {app.category}
                </h3>
                <p className={`text-sm transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-700'}`}>
                  {app.examples}
                </p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ============ TURNAROUND TIMES ============ */}
      <section className={`relative py-24 px-4 sm:px-6 lg:px-8 transition-colors duration-300 ${isDark ? 'bg-slate-950' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto">
          <h2 className={`text-5xl font-black mb-16 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
            Test Turnaround Times
          </h2>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {turnaroundTimes.map((item, idx) => (
              <div
                key={idx}
                className={`rounded-2xl border p-6 transition-all duration-300 ${
                  isDark
                    ? 'bg-white/5 border-white/10 hover:border-emerald-500/50'
                    : 'bg-gradient-to-br from-emerald-50 to-cyan-50 border-emerald-200/60 shadow-sm'
                }`}
              >
                <Clock className="w-8 h-8 text-emerald-600 mb-3" />
                <h3 className={`font-bold mb-2 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {item.testCategory}
                </h3>
                <p className={`text-lg font-bold text-emerald-600 mb-2 transition-colors duration-300`}>
                  {item.time}
                </p>
                <p className={`text-xs transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-700'}`}>
                  {item.examples}
                </p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ============ QUALITY ASSURANCE ============ */}
      <section className={`relative py-24 px-4 sm:px-6 lg:px-8 transition-colors duration-300 ${isDark ? 'bg-slate-900/50' : 'bg-gray-50'}`}>
        <div className="max-w-7xl mx-auto">
          <h2 className={`text-5xl font-black mb-16 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
            Quality Assurance Standards
          </h2>

          <div className="grid md:grid-cols-3 gap-8">
            {qualityAssurance.map((qa, idx) => (
              <div
                key={idx}
                className={`rounded-2xl border p-8 transition-all duration-300 ${
                  isDark
                    ? 'bg-gradient-to-br from-white/10 to-white/5 border-white/15'
                    : 'bg-white border-gray-200 shadow-sm'
                }`}
              >
                <h3 className={`text-lg font-bold mb-4 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {qa.measure}
                </h3>

                <ul className="space-y-2">
                  {qa.items.map((item, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-600 flex-shrink-0 mt-1.5"></div>
                      <span className={`text-xs transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-700'}`}>
                        {item}
                      </span>
                    </li>
                  ))}
                </ul>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ============ TEAM SECTION ============ */}
      <section className={`relative py-24 px-4 sm:px-6 lg:px-8 transition-colors duration-300 ${isDark ? 'bg-slate-950' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto">
          <h2 className={`text-5xl font-black mb-16 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
            Clinical Team
          </h2>

          {teamMembers.map((doctor, idx) => (
            <div
              key={idx}
              className={`rounded-3xl border overflow-hidden transition-all duration-300 max-w-5xl mx-auto ${
                isDark
                  ? 'bg-gradient-to-r from-white/8 to-white/3 border-white/10'
                  : 'bg-white border-gray-200 shadow-lg'
              }`}
            >
              <div className="h-2 bg-gradient-to-r from-emerald-600 to-cyan-600"></div>
              
              <div className="p-8">
                <div className="grid md:grid-cols-2 gap-8 items-center mb-8">
                  <div>
                    <div className="w-32 h-32 rounded-2xl flex items-center justify-center text-6xl bg-gradient-to-br from-emerald-600 to-cyan-600 mb-6">
                      👨‍⚕️
                    </div>
                    <h3 className={`text-3xl font-black mb-2 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {doctor.name}
                    </h3>
                    <p className="text-emerald-600 font-bold text-lg mb-3">{doctor.specialization}</p>
                    <div className={`space-y-1 transition-colors duration-300 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                      <p><strong>Qualifications:</strong> {doctor.qualification}</p>
                      <p><strong>Experience:</strong> {doctor.experience}</p>
                      <p><strong>Availability:</strong> {doctor.availability}</p>
                    </div>
                  </div>

                  <div>
                    <h4 className={`text-xl font-bold mb-4 transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      Areas of Expertise
                    </h4>
                    <ul className="space-y-2">
                      {doctor.expertise.map((exp, i) => (
                        <li key={i} className="flex items-center gap-2">
                          <div className="w-2 h-2 rounded-full bg-emerald-600"></div>
                          <span className={`text-sm transition-colors duration-300 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                            {exp}
                          </span>
                        </li>
                      ))}
                    </ul>

                    <button className="w-full mt-6 px-6 py-3 rounded-full bg-gradient-to-r from-emerald-600 to-cyan-600 text-white font-bold hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 group">
                      Schedule Consultation
                      <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                    </button>
                  </div>
                </div>
              </div>
            </div>
          ))}
        </div>
      </section>

      {/* ============ CTA SECTION ============ */}
      <section className={`relative py-24 px-4 sm:px-6 lg:px-8 overflow-hidden bg-gradient-to-r from-emerald-600 to-cyan-600`}>
        <div className="absolute inset-0">
          <div className="absolute -top-40 -right-40 w-96 h-96 bg-white/10 rounded-full blur-3xl"></div>
          <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-white/10 rounded-full blur-3xl"></div>
        </div>

        <div className="relative max-w-4xl mx-auto text-center">
          <h2 className="text-5xl font-black text-white mb-8">
            Your Trusted Diagnostic Partner
          </h2>
          <p className="text-xl text-white/90 mb-12">
            Advanced laboratory services with rapid turnaround times, accurate results, and expert interpretation for comprehensive health assessment.
          </p>

          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/"
              className="inline-flex items-center justify-center gap-2 px-10 py-4 bg-white text-emerald-600 font-bold rounded-full hover:shadow-2xl transition-all duration-300 hover:scale-105 group"
            >
              <Calendar className="w-5 h-5" />
              Book Test
              <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Link>
            <a
              href="tel:+919876543210"
              className="inline-flex items-center justify-center gap-2 px-10 py-4 border-2 border-white text-white font-bold rounded-full hover:bg-white/10 transition-all duration-300"
            >
              <Phone className="w-5 h-5" />
              Contact Us
            </a>
          </div>
        </div>
      </section>
    </div>
    </Layout>
  );
}
